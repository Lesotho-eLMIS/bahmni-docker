...
class ApiEventWorkerExt(models.AbstractModel):
    _inherit = "api.event.worker"
    ...
    def _get_customer_vals(self, vals):
        ...
        _logger.debug(
            "Lesotho Bahmni API Feed: Base customer_vals from Bahmni: %s", customer_vals
        )
        ...
        if sex:
            ...
            _logger.info(
                "Lesotho Bahmni API Feed: Added sex=%s for ref=%s", sex, vals.get("ref")
            )
        else:
            _logger.debug(
                "Lesotho Bahmni API Feed: No sex field present in payload for ref=%s",
                vals.get("ref"),
            )
        ...
        if age is not None:
            try:
                ...
                _logger.info(
                    "Lesotho Bahmni API Feed: Added age=%s for ref=%s",
                    age,
                    vals.get("ref"),
                )
            except ValueError:
                _logger.warning(
                    "Lesotho Bahmni API Feed: Invalid age received (%s) for ref=%s",
                    age,
                    vals.get("ref"),
                )
        else:
            _logger.debug(
                "Lesotho Bahmni API Feed: No age field present in payload for ref=%s",
                vals.get("ref"),
            )

        _logger.debug(
            "Lesotho Bahmni API Feed: Final customer_vals to be saved: %s",
            customer_vals,
        )
